		...
		if (username == 1){ //option for if user is an admin
			...
			while (userInput9 != 7){
				...
				while(true){
					...
					catch(InputMismatchException inputLoop){
				...
				if (userInput9 == 1){ //play first movie of wishlist
					...
					if (! customerDatabase.findCustomerWishList(username).wishListEmpty()){
						...
						if (play.haveMovie() == true){
							...
							//add the movie to the recently watched stack
							customerDatabase.findRecentlyWatchedStack(username).push(play);
							...
							System.out.println("\nSorry that movie is no longer in stock.\n");
							customerDatabase.findCustomerWishList(username).delete(play.getID());
						}
					}
					else{
					...
					while(true){
						...
						catch(InputMismatchException otherInputThing){
					...
					customerDatabase.findRecentlyWatchedStack(username).printRecentlyWatchedStack();
				}
